	...
	if (filter_var($ntp_server,FILTER_VALIDATE_IP) || preg_match('/^(([a-z0-9]|[a-z0-9][a-z0-9\-]*[a-z0-9])\.)*([a-z0-9]|[a-z0-9][a-z0-9\-]*[a-z])$/i',$ntp_server)) {
		...
		if ($ntp_time > 0) {
			...
			if ($diff_time < -600 || $diff_time > 600) {
				$result['alarm'] = "red";
				$result['data'] = date("Y-m-d H:i:s") . " (Please check time. It is different (more than 10 minutes) from NTP server $ntp_server)";
				...
				$result['data'] = date("Y-m-d H:i:s") . " (Localtime is equal to NTP server $ntp_server)";
			}
		} else {
			$result['alarm'] = "red";
